...
return (
  ...
  <div className="card">
    <ul>
      ...
      <button className="btnpopup" onClick={() => { setpopupOpen(popupOpen ? false : true) }}>להצגת כל הפרטים לבית קפה</button>
      <button className="btnorders" onClick={() => { setordersOpen(ordersOpen ? false : true) }}>להזמנת שולחן בבית קפה</button>
      <div className="popupform">{ordersOpen &&
        <form className="ordersform" onSubmit={(e) => { submitOrder(e) }}>
          ...
          <label className='orderslbl'>:מספר טלפון 
            <input className="ordersinput" type="text" id="customertel" placeholder="מספר טלפון" name="customerphone" required/>
          </label>
          </div>
